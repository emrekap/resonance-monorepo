...
| **Red**    | No lift over metadata+text — the 95% CI on the uplift includes 0, **or lies entirely below 0** (B3 measurably worse than the baseline) | Neuro-features are not earning their GPU cost — pivot the technical story before scaling spend |

Committing to the **Red** action in advance is the only thing that makes Green mean anything.

**Precedence is Red-first.** As worded, these three bands are not mutually exclusive: a positive
point estimate below 0.10 whose CI still includes zero satisfies both Yellow's text and Red's. Red is
decided first — an uplift whose CI includes zero, or lies entirely below zero, is Red regardless of
the point estimate; Green and Yellow are only considered once Red has been ruled out. See
...
- **Regime 1 — new post, known creator (headline):** per creator, temporal split; train on older
  ...
  band is **the** Verdict in §6 and §8 — named in advance, before any data existed, so the headline
  cannot be chosen after seeing which regime looks better. See [Amendment 2](#amendments).
...
- **Negative-control false-positive rate** — measured on the harness's own synthetic worlds, not on
  ...
  sitting right on the boundary (−0.100, −0.102, −0.145, +0.105). Both thresholds are round numbers
...
**Amendment 1 (2026-08-09) — §6 gains a fourth case: a wholly-negative uplift CI is Red, not Yellow.**
As originally written, §6's three bands did not cover a 95% CI on the uplift that lies _entirely
below_ zero — B3 measurably worse than the baseline. That outcome matched neither Red ("CI includes
0") nor Green, so it fell through to Yellow ("iterate and re-run"), prescribing another run for the
clearest refutation the experiment can produce. `research/eval/verdict.py`'s `_is_worse_than_baseline`
now classifies it Red, checked after the two existing Red checks and before Green — precedence
remains Red-first, nothing reordered. **Attestation:** found and fixed against the null synthetic
world only; no real-cohort or test-set data existed or was observed at any point during this change.
...
headline. **Attestation:** made before either regime's result — real or synthetic-final — was
compared for this purpose; the two can legitimately disagree on synthetic worlds by construction,
which is what surfaced the gap this amendment closes.
